...
func InputJsonFileToInputStruct(filename string, projectName string) (metadata.Project, string) {
	...
	input.Env = project.EnvInit(input.Env, projectName)
	return input, ""
}
...
func InputStructToOutputString(templateFolder string, input metadata.Project, result *string, language string) (metadata.Output, string) {
	...
	templateMap, err := generator.TemplateMapInitF(templateFolder, input)
	if err != nil {
...
func OutputStructToFiles(directory string, output metadata.Output) string {
...
func GenerateFromFile(temp, project, filename string, outputString *string, language string) (metadata.Output, string) {
	var output metadata.Output
	if temp != "" {
		templateDir = temp
	} else {
	...
	input, err := InputJsonFileToInputStruct(filename, projectName)
	if err != "" {
		return output, err
	}
	output, err = InputStructToOutputString(templateDir, input, outputString, language)
	if err != "" {
		return output, err
	}
	return output, ""
